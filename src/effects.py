...
class BgStar(Sprite):
    def __init__(self):
        ...
        self.image = Surface((30, 150))
        self.image.set_colorkey((0, 0, 0))
        self.rect = self.image.get_rect()
        star_size = randint(1, 3)
        x = randint(0, self._screen_size.x)
        speed = randint(5, 10)
        # pygame.draw.circle(self.image, (255, 255, 255), self.rect.center, star_size)
        pygame.draw.line(self.image, (255, 255, 255, 25), (0, 0), (0, 150), star_size)
    ...
    def update(self):
        ...
        if self.rect.top > self._screen_size.y:
            self.kill()
...
class Explosion(Sprite):
    def __init__(self, pos, size=1):
        super().__init__()
        self.images = [
            load_and_resize(f"assets/imgs/effects/explosion_{n:02d}.png", (30*size, 30*size))
            for n in range(1, 6)
        ]
